...
-(HTTPHandler*) executeHTTPRequest:(NSString *)aUri httpmethod:(NSString *)method httpbodydata:(NSData *)body etag:(NSString *)etag customHeaders:(NSMutableDictionary *)custom_headers
{
	...
	{
		id key, value;
		NSArray *keys = [m_customHeaders allKeys];
		int count = [keys count];
		for (int i = 0; i < count; i++)
		{
			key = [keys objectAtIndex: i];
			value = [m_customHeaders objectForKey: key];
			[headers setObject:value
						forKey:key];
		}	
	}
...
- (void) addHeader:(NSString*) aHeaderName headerValue:(NSString*)aHeaderValue
{
    ...
    //NSLog(@"\n PINE DEBUG -> m_customHeaders count = %d\n", [m_customHeaders count]);
}
...
{
    /*NSString *accessToken = @"MTIzOm5MWUxRVnNKSTFpTytPejNpbkV3S0E9PQ==";//[defaults objectForKey:@"ACSUserToken"];
    HTTPHeaders *headers = [request getHeaders];
    NSMutableDictionary *headersDict = [headers getHttpHeaders];
    [headersDict setObject:[NSString stringWithFormat:@"Basic %@", accessToken] forKey:@"Authorization"];
	...
	{
        NSLog(@"\n PINE DEBUG -> onBeforeHttpRequest OK\n");
