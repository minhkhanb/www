...
#define SERVER_REQUEST_CODE_LOAD_FILE			16
#define SERVER_REQUEST_CODE_FB_POST_PHOTO		17
//new system task
#define SERVER_REQUEST_CODE_SHARE_LINK_TASK			22
#define SERVER_REQUEST_CODE_SHARE_MEDIA_TASK		23
		...
		static int Request(uint32 request_code){
			...
			for(int i = 0; i< instance->StreamNum; i++){
				if(instance->Streams[i]->Status == SERVER_STREAM_STATUS_FREE){
					...
					handle = i;
					ServerStreamPreRequest(i,request_code);
					PineRenderer::getInstance()->CallBaseService(CONNECTOR_SERVER_CHECK_AND_RUN, NULL, 0);
					break;
				}
			}
			return handle;
			...
			if(filename != nullptr && filename->Length > 0){
				...
				for(int i = 0; i< instance->StreamNum; i++){
					if(instance->Streams[i]->Status == SERVER_STREAM_STATUS_FREE){
						...
						handle = i;
						ServerStreamPreRequest(i,SERVER_REQUEST_CODE_FB_GET_AVATAR);
						PineRenderer::getInstance()->CallBaseService(CONNECTOR_SERVER_CHECK_AND_RUN, NULL, 0);
						break;
					}
				}
