...
namespace ToolStackPNGWriterLib
{
    ...
    public static class PNGWriter
    {
        ...
        */
        ...
        public static void WritePNG(Stream raw_stream, Stream out_stream, int width, int height)
        {
            _stream = out_stream;
            // Write the png header.
            out_stream.Write(
            ...
            raw_stream.Read(colors, 0, width * height * 4);
            /*for (int i = 0; i < colors.Length - 1; i += 4)
            {
                byte A = colors[i];
                byte R = colors[i + 1];
                byte G = colors[i + 2];
            ...
            // Write PNG without any compression
            WriteDataChunksUncompressed(colors, width, height);
        ...
        private static void WriteDataChunksUncompressed(byte[] colors, int width, int height)
        {
            try
            {
                // First setup some variables we're going to use later on so we can calculate how big of byte[] we need 
                ...
                // Figure out how much image data we're going to have:
                //      H * W * (number of bytes in an ARGB value) + H to account for the filter byte in PNG files
                int dataLength = width * height * 4 + height;
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    for (int x = 0; x < width; x++)
                    {
                        ...
                        //pixel = BitConverter.GetBytes(_image.Pixels[pixelSource]);
                        pixel = new byte[4];
                        //pixel[0] = colors[pixelSource * 4 + 3];
                        //pixel[1] = colors[pixelSource * 4 + 1];
                        //pixel[2] = colors[pixelSource * 4 + 2];
                        //pixel[3] = colors[pixelSource * 4 + 0];

                        pixel[0] = colors[pixelSource * 4 + 1];//red
                        pixel[1] = colors[pixelSource * 4 + 2]; //green
                        pixel[2] = colors[pixelSource * 4 + 3]; //blue
                        pixel[3] = colors[pixelSource * 4 + 0]; //anpha
                        // Loop through the 4 bytes of the pixel and 'write' them to the data array.
                        for (int z = 0; z < 4; z++)
                        {
                            // This is the second appearance of this code code.
