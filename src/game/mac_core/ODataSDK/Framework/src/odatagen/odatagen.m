...
/*************************************************************************************************************************************************************************************
 * DESCRIPTION  : Function to create implementation file with extension .h. This will include at ObjectContent.h to get metatadata dto class for Odata V3
 * PARAMETER    : xml file which is to be parsed, output file name with path
 * RETURN VALUE : void
 *************************************************************************************************************************************************************************************/

void CreateIncludeHFile(const char * xmlfilename,const char* OutFileName)
{
    xsltStylesheetPtr cur = NULL;
	xmlDocPtr doc, res;FILE *fp;
	...
	dirPath = [mainPath  stringByAppendingString:@"/objc_externMetaData.xsl"];
	exists = [fileManager fileExistsAtPath:dirPath isDirectory:&isDir];
	if (exists) {
		cur = xsltParseStylesheetFile((const xmlChar *)[dirPath UTF8String]);// .xsl
		doc = xmlParseFile(xmlfilename);//Parse the Input File
		...
		@throw [NSException exceptionWithName:@"File Not Found" reason:@"objc_externMetaData.xsl file not found" userInfo:nil];
	}
...
/*************************************************************************************************************************************************************************************
 * DESCRIPTION  : Function to create implementation file with extension .m. This will include at ObjectContent.m to get metatadata dto class for Odata V3
 * PARAMETER    : xml file which is to be parsed, output file name with path
 * RETURN VALUE : void
 *************************************************************************************************************************************************************************************/

void CreateIncludeMFile(const char * xmlfilename,const char* OutFileName)
{
...
int main (int argc, const char * argv[])
{
	...
	@try{
		...
		if (range.location != NSNotFound) {
            ...
            /*if (![fileManager fileExistsAtPath:[mainPath stringByAppendingString:@"/objc_externMetaData.xsl"] isDirectory:&isDir])
                [fileManager copyItemAtPath:[[mainPath substringToIndex:fileRange.location] stringByAppendingString:@"objc_externMetaData.xsl"] toPath:[mainPath stringByAppendingString:@"/objc_externMetaData.xsl"] error:&err];*/
			...
			if (err) 
                ...
                /*if (![fileManager fileExistsAtPath:[mainPath stringByAppendingString:@"/objc_externMetaData.xsl"] isDirectory:&isDir])
                    [fileManager copyItemAtPath:[filePath stringByAppendingString:@"/objc_externMetaData.xsl"] toPath:[mainPath stringByAppendingString:@"/objc_externMetaData.xsl"] error:&err];*/
		...
		if(headers!=nil)
		{
			...
			if([headers objectForKey:@"/uri"])
			{
				...
				else
				{
					...
					if([filename length]!=0)
					{
                        ...
                        /*outFileName=[outDirPath stringByAppendingString:@"_extern.h"];
						outFile=[outFileName cStringUsingEncoding:NSUTF8StringEncoding];
                        CreateIncludeHFile("./metadata.xml", outFile);*/
			...
			else if([headers objectForKey:@"/metadata"])
			{
				...
				if([filename length]!=0)
				{
                    ...
                    /*outFileName=[outDirPath stringByAppendingString:@"_extern.h"];
					outFile=[outFileName cStringUsingEncoding:NSUTF8StringEncoding];
					CreateIncludeHFile(path,outFile);*/
				}
				else
				{
					NSLog(@"odatagen: Failed to generate proxy classes. Please verify the metadata.");
