...
+(void)setObjectInstanceVariable:(id) object varname:(NSString *)variablename varval:(NSString *)value 
{
	...
	else if(strcmp(type,"@\"NSDate\"") == 0 )
	{
		NSRange range = [value rangeOfString:@"Date"];
	        ...
	        if (range.location == NSNotFound) //XML
	        {
	            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
	            [dateFormat setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
	            range = [value rangeOfString:@"."];
	            if (range.location != NSNotFound) {
	                NSLocale *USPOXIS = [[[NSLocale alloc] initWithLocaleIdentifier:@"en_US"] autorelease];
	                [dateFormat setLocale:USPOXIS];
	            ...
	            //NSLog(@"\n date in ODATA %@, value = %@ ", date, value);
	            [dateFormat release];
	            objectValue = date;
	        }
	        else //JSON
	        {
	            range = [value rangeOfString:@")/"];
	            value = [value substringWithRange:NSMakeRange(6, range.location - 6)];
	            //NSLog(@"\n value = %@, range = %d ", value, range.location);
...
+(NSString *)getObjectInstanceVariableValue:(id)object variablename:(NSString *)varname isComplex:(ODataBool *)complex 
{
	...
	else if(strcmp(type,"@\"NSNumber\"") == 0 )
	{
		...
		if(number)
        {
            ...
            {
                value = [[NSString alloc] initWithFormat:@"%lld", [number longLongValue]];
            }
            else
            {
	...
	else if(strcmp(type,"@\"NSDate\"") == 0)
	{
		...
		[dateFormat setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
        	...
        	[dateFormat setLocale:USPOXIS];
		[dateFormat setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss'Z'"];
		//[dateFormat setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss.fffffff"];
        	//NSLog(@"\n dateTime = %@",dateTime);
