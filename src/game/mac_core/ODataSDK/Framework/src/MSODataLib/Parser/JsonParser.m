...
-(id)initwithContext:(ObjectContext *)context
{
	 ...
	 * return NSString
	 */
-(NSString *)getEditLink:(NSDictionary *)theElement
{
    ...
    //get editLink
    NSString *theMetaData = [theElement objectForKey:@"odata.metadata"];
    
    int beginSub = [theMetaData rangeOfString:@"#"].location+1;
    entity = [theMetaData substringFromIndex:beginSub];
    NSRange endRange = [entity rangeOfString:@"/"];
    if (endRange.length > 0)
    {
        entity = [entity substringToIndex:endRange.location];
    }
    uri = [theMetaData substringToIndex:beginSub];
    uri = [uri stringByReplacingOccurrencesOfString:@"$metadata#" withString:@""];
    uri = [uri stringByAppendingString:entity];
    entity = [NSString stringWithString:[m_objectContext getDtoNameFromMetaData:entity] ];
    //NSLog(@"\n entity = %@ \n", entity);
	id resource = nil;

    resource = [[NSClassFromString(entity) alloc] initWithUri:uri];
    
    NSString *typeEdm = [[(ODataObject*)resource getPropertiesFromPropertiesMap:@"m_ID"] getEdmType];
    NSString *valueId = [NSString stringWithFormat:@"%@", [theElement objectForKey:@"ID"]];

    if ([typeEdm compare:@"Edm.Int64"] == NSOrderedSame)
    {
        uri = [uri stringByAppendingString:@"("];
        uri = [uri stringByAppendingString:valueId];
        uri = [uri stringByAppendingString:@"L)"];
    }
    else if ([typeEdm compare:@"Edm.Guid"] == NSOrderedSame)
    {
        uri = [uri stringByAppendingString:@"(guid'"];
        uri = [uri stringByAppendingString:valueId];
        uri = [uri stringByAppendingString:@"')"];
    }
    else if ([typeEdm compare:@"Edm.String"] == NSOrderedSame)
    {
        uri = [uri stringByAppendingString:@"('"];
        uri = [uri stringByAppendingString:valueId];
        uri = [uri stringByAppendingString:@"')"];
...
-(void)updateEntryObjectsWithJson:(NSDictionary *)theElement resourceBox:(ResourceBox *)aResourceBox
{
    id obj = nil;
    NSString *editLink = [self getEditLink:theElement];
    ...
    {        
        [aResourceBox setEditLink:editLink];
        [self storeObjectProperties:obj dataElement:theElement];
    }
    else
        return;
}
...
/*
 * Enumerate and Parse json elements
 * @param NSMutableArray Json element
 * @param id class instance of ObjectContext
 * @param id class instance of QueryOperationResponse
 * return NULL
 */
-(void)EnumerateObjects:(NSDictionary*)jsonDocument queryResponseObject:(id)queryResponse
{
    ...
    NSDictionary *data = [jsonDocument objectForKey:@"value"];
    ...
    {
        [m_queryResponseObject setInlineCountValue:data.count];
    }
    data = jsonDocument;
    ...
    entries = [self EnumerateFeeds:data];
    
    if (!entries)
        entries = [[NSMutableArray alloc] initWithCapacity:0];
    
    NSInteger resultCount = [entries count];
    
    [m_queryResponseObject setCountValue:resultCount];
...
/*
 * Enumerate Feed elements from JSON
 * @param NSDictionary json element000000000
 * return NSMutableArray
 */
-(NSMutableArray *) EnumerateFeeds:(NSDictionary *)theElement;
{
	NSMutableArray *class_objects = nil;
    
    NSString *odataMetaData = [theElement objectForKey:@"odata.metadata"];
    
    NSArray *data = [theElement objectForKey:@"value"];
    
    if (data) // multi record
    {
        for (int i = 0; i < [data count]; i++)
        {
            id entry = [self EnumerateEntry:[data objectAtIndex:i] odataMetadata:odataMetaData];
            if (entry != nil)
            {
    ...
    {
        id entry = [self EnumerateEntry:theElement odataMetadata:odataMetaData];
        if (entry != nil)
        {
            if(class_objects == nil)
                class_objects = [[NSMutableArray alloc] init];
    ...
    /*for (int i = 0; i < [theElement count]; i++)
    {
        id entry = [self EnumerateEntry:[theElement objectAtIndex:i]];
        if (entry != nil)
        {
            if(class_objects == nil)
                class_objects = [[NSMutableArray alloc] init];
        ...
        temp = [NSString stringWithFormat:@"%@", [theElement objectAtIndex:i]];
        NSLog(@"\n %d -----> \n %@ \n", i, temp);
    }*/
 ...
 * return id
 */
-(id)EnumerateEntry:(NSDictionary *)entry odataMetadata:(NSString*)theString;
{
    id classObject = [m_objectContext addToObjectToResource:entry odataMetaData:theString];
    
	if(classObject == nil)
	{
		return nil;
	}
    [self storeObjectProperties:classObject dataElement:entry];
	return classObject;
}
 ...
 * return NULL
 */
-(void)storeObjectProperties:(id)dynamicObject dataElement:(NSDictionary *)theElement;
{
	//Get property
	NSArray* arrayKey = [theElement allKeys];
    
	int count = [arrayKey count];
    ...
    for(int i = 0; i < count; i++)
    {
        nodeName = [NSString stringWithFormat:@"%@", [arrayKey objectAtIndex:i]];
        if (![nodeName isEqualToString:@"odata.metadata"])
        {
            node = [theElement objectForKey:nodeName];
            tag = [@"m_" stringByAppendingString:nodeName];
            [[ODataRTTI getObjectInstanceVariable:dynamicObject variablename:tag] autorelease];
            [ODataRTTI setObjectInstanceVariable:dynamicObject varname:tag varval:node];
        }
    }
...
/*
 * Parse and build OData JSON from class Object
 * @param id class Object
 * @param char method type to use
 * return NSString
 */
-(NSString *)buildJSON:(id) object methodtype:(const char *)type
{
    ...
    NSMutableDictionary *ske = [[NSMutableDictionary alloc] initWithCapacity:[variablenames count]];
    ...
    for (int i=0; i<[variablenames count]; i++)
	{
        ...
        value = [self retriveObjectJSONValue:object methodtype:type variablename:stringFromUTFString];
        key = [stringFromUTFString substringWithRange: NSMakeRange(2,[stringFromUTFString length]-2)];
        if (value != nil)
        {
            [ske setObject:value forKey:key];
        }
    }
    NSString * ret = [[NSString alloc] initWithData:[NSJSONSerialization dataWithJSONObject:ske options:NSJSONWritingPrettyPrinted  error:nil] encoding:NSUTF8StringEncoding];
 ...
 * return NSString
 */
-(NSString *)retriveObjectJSONValue:(id)object methodtype:(const char *)methodtype variablename:(NSString *)varname
{
    NSString *value = nil;
	ODataBool *isComplex = [[ODataBool alloc] init];
