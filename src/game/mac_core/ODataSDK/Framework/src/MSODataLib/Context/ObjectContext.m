...
- (id) initWithUri: (NSString*) aUri credentials:(id)aCredential dataServiceVersion:(NSString *)aDataServiceVersion
{
	if(self = [super init] )
	{
		...
		self.m_customHeaders			= [[NSMutableDictionary alloc] init];
        	[self setParseData:XML_Parser_Type];
...
-(HTTPHandler*) executeHTTPRequest:(NSString *)aUri httpmethod:(NSString *)method httpbodydata:(NSData *)body etag:(NSString *)etag customHeaders:(NSMutableDictionary *)custom_headers
{
	...
	NSMutableDictionary *headers = [[Utility CreateHeaders:method eTag:etag ODataServiceVersion:[self getDataServiceVersion]] retain];
   	...
   	//hao.ly modify for proxy header
  	{
		id key;
		...
		for (int i = 0; i < count; i++)
		{
			...
			if ([key  isEqual: @"Accept"])
			{
				[headers setObject:m_accept forKey:@"Accept"];
			}
			if ([key  isEqual: @"Content-Type"])
			{
				[headers setObject:m_contentType forKey:@"Content-Type"];
			}
		        if ([key isEqual:@"DataServiceVersion"])
		        {
		        	[headers setObject:m_dataServiceVersion forKey:@"DataServiceVersion"];
		        }
		}
    	}
	...
	if([httpRequest http_error]){
        //NSLog(@"\nPINE DEBUG http_error %d ; %@\n", [response getCode], [[httpRequest http_error] localizedDescription]);
...
- (ODataObject*) addToObjectToResource:(NSDictionary*)anEntry odataMetaData:(NSString*)theMetaData;
{
    NSString *uri = nil;
    NSString *entity = nil;
    ...
    entity = [NSString stringWithString:[self getDtoNameFromMetaData:entity]];
    ...
    @try
    {
        ...
        NSString *valueId = [NSString stringWithFormat:@"%@", [anEntry objectForKey:@"ID"]];
...
//JSON parse
-(void)setParseData:(enum ParseDataType)parserType
{
    m_ParseDataType = parserType;
    switch (m_ParseDataType) {
        case XML_Parser_Type:
            [self setAccept:[NSString stringWithString:Resource_Accept_ATOM]];
            [self setContentType:[NSString stringWithString:Resource_Content_Type_ATOM]];
            [self setDataServiceVersion:@"2.0"];
            break;
        case JSON_Parser_Type:
            [self setAccept:[NSString stringWithString:Resource_Accept_JSON]];
            [self setContentType:[NSString stringWithString:Resource_Content_Type_JSON]];
            [self setDataServiceVersion:@"3.0"];
            break;
    }
    
}

-(enum ParseDataType) getParseData
{
    return m_ParseDataType;
}
