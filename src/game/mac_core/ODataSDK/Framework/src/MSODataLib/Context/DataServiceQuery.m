...
- (DataServiceQuery*) addQueryOption:(NSString*)anOption query:(NSString*)aQuery
{
    ...
    else if([tmpOption isEqualToString:@"@custom"])
    {
        [m_other addObject:[NSString stringWithFormat:@"%@", tmpQuery]];
    }
	else
	{
...
/**
 * To add custom query option.
 *
 * @return <DataServiceQuery> Self reference that includes the requested
 *                            inlinecount=allpages query option
 * @throws DataServiceRequestException
 */
- (DataServiceQuery*) custom:(NSString*)aCustom
{
    return [self addQueryOption:@"@custom" query:aCustom];
}
...
- (QueryOperationResponse*) execute:(NSString*) aQuery
{
    ...
    //NSLog(@"\nPine debug query = %@", query);
    if (aQuery)
    {
        query = [query stringByAppendingFormat:@"%@",aQuery];
    }
    
	NSString *queryOption=[self buildQueryOption];
    //NSLog(@"\nPine debug queryOption = %@", queryOption);
	if([queryOption length] > 0){
		query = [query stringByAppendingFormat:@"?%@",queryOption];	
    	}

	HTTPHandler *httpRequest = [m_context executeHTTPRequest:query httpmethod:@"GET" httpbody:nil etag:nil /*customHeaders:nil*/];
	...
	switch ([m_context getParseData]) {
        ...
        {
            NSError *error = nil;
            NSDictionary *object = [NSJSONSerialization JSONObjectWithData:nonUTF8Data options:NSJSONReadingMutableContainers error:&error];
            
            if (object && error == nil)
            {
                JsonParser *json = [[JsonParser alloc] initwithContext:m_context];
                [json EnumerateObjects:object queryResponseObject:queryOperationResponse];
                [json release];
            }
