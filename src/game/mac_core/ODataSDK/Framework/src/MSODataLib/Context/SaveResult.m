...
-(NSString *) createRequestBody:(ResourceBox *)aResourceBox replacementOption:(BOOL)aReplaceOnUpdateOption
{
    ...
    if ([self getContext].getParseData == XML_Parser_Type)
    {
        AtomParser *atom = [[AtomParser alloc] initwithContext:m_context];
    ...
    else if ([self getContext].getParseData == JSON_Parser_Type)
    {
        JsonParser *json = [[JsonParser alloc] initwithContext:m_context];
        
        if (state == Added)
        {
            str = [json buildJSON:object methodtype:"POST"];
        }
        else if (state == Modified)
        {
            NSString * entityHttpMethod = [self getEntityHttpMethod:state replaceOnUpdateOption:aReplaceOnUpdateOption];
            str = [json buildJSON:object methodtype:[entityHttpMethod UTF8String]];
        }
...
-(void)processObject:(ResourceBox *)aResourceBox replacementOption:(BOOL)aReplaceOnUpdateOption
{
    ...
    else if ([[self getContext] getParseData] == JSON_Parser_Type)
    {
        ...
        NSError *error = nil;
        NSDictionary *object = [NSJSONSerialization JSONObjectWithData:[response dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:&error];
        if (object && error == nil)
        {
            [json updateEntryObjectsWithJson:object resourceBox:aResourceBox];
        }
        if (error != nil)
        {
            [error release];
