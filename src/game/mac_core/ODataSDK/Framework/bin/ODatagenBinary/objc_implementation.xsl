...
xmlns:schema_1_1="http://schemas.microsoft.com/ado/2009/11/edm"
...
@end
 ...
 <!-- Generate function Dto   -->
<xsl:template match="schema_1_0:EntityMatchContainer | schema_1_1:EntityMatchContainer | schema_1_2:EntityMatchContainer">
...
<xsl:variable name = "modifyText1" select="substring-after($Text, 'DataTransferObjects.')"></xsl:variable>
<xsl:variable name = "modifyText2" select="substring-before($modifyText1, 'Dto')"></xsl:variable>
      return @"ODataObject_<xsl:value-of select="$modifyText2"/>";
  }
</xsl:for-each>  
    return nil;
}
...
<xsl:variable name="ObjectName" select="concat('ODataObject_', substring-before($ClassName, 'Dto'))"/>
 ...
 */
 @implementation <xsl:value-of select="$ObjectName"/>
...
{
	<xsl:value-of select="$ObjectName"/> *a<xsl:value-of select="@Name"/> = [[<xsl:value-of select="$ObjectName"/> alloc]init];
...
-(NSMutableArray *)getSyndicateArray
{
	...
	-(<xsl:value-of select="$ObjectName"/> *)getDeepCopy
{
<xsl:value-of select="$ObjectName"/> *obj=[[<xsl:value-of select="$ObjectName"/> alloc]initWithUri:[self getBaseURI]];
