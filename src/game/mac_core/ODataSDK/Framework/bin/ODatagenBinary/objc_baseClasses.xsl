...
<xsl:variable name="ObjectName" select="concat('ODataObject_', substring-before(@Name, 'Dto'))"/>
<xsl:variable name="service_namespace" select="concat(//schema_1_0:EntityType/../@Namespace, //schema_1_1:EntityType/../@Namespace, //schema_1_2:EntityType/../@Namespace)" />
/**
 * @interface:<xsl:value-of select="@Name"/>
 * @Type:EntityType
 ...
 */
@interface <xsl:value-of select="$ObjectName"/> : <xsl:choose><xsl:when test="@BaseType"><xsl:value-of select="substring-after(@BaseType, concat($service_namespace, '.'))"/></xsl:when><xsl:otherwise>ODataObject</xsl:otherwise></xsl:choose>
{
	<xsl:for-each select="schema_1_0:Property | schema_1_1:Property | schema_1_2:Property">
	/**
