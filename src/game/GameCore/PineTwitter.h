...
#define REQUIRES_TWITTER_LOGIN "requires Twitter account to back up your data online. Check your device setting for logging in and re-launch the app to secure your progress"
...
-(void) TwitterInit
{
    if(![self userHasAccessToTwitter])
    {
        ...
        CGame::_this->_pine_social->SetFlag(SOCIAL_FLAG_NO_ACCOUNT_LOGIN);
        char strReqires[512];
        sprintf(strReqires,"%s %s",GAME_NAME_REQUIRE_LOGIN,REQUIRES_TWITTER_LOGIN);
        PineSocial::ShowMessageBox(strReqires);
        return ;
    }
            ...
            int count = [arrayOfAccounts count];
             if (count > 0) {
                ...
                NSDictionary *properties = [twitterAccount dictionaryWithValuesForKeys:[NSArray arrayWithObject:@"properties"]];
                 ...
                 CGame::_this->_pine_social->SetInfomation([[NSString stringWithFormat:@"%@",[details objectForKey:@"user_id"]] UTF8String],[[twitterAccount.username retain] UTF8String]);
#else
...
-(void)getTwitterFriendsForAccount:(ACAccount*)account {
    ...
    [twitterRequest performRequestWithHandler:^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error) {
        ...
        {
#if USE_PINESOCIAL_AT_ID_STRING
            CGame::_this->_pine_social->AddFriend([[NSString stringWithFormat:@"%@",[list_id objectAtIndex:i]] UTF8String]);
#else
    ...
    [tw_waccountStore requestAccessToAccountsWithType:accountType options:nil completion:^(BOOL granted, NSError *error)
     {
         ...
         if (granted == YES)
         {
             ...
             if (count > 0)
             {
                 ...
                 NSDictionary *params = @{@"status" : [NSString stringWithFormat:@"%@ %s https://itunes.apple.com/us/app/id%d ",feed,TAG_LINE,APPLE_ID]};
...
-(void) PostImageToTwitter:(NSString*) feed withRealUrl:(NSString*)url
{
    if(![self userHasAccessToTwitter])
    {
        char strReqires[512];
             ...
             if (count > 0)
             {
                 ...
                 @try
                 {
                     ...
                     else
                     {
                         ...
                         UIGraphicsBeginImageContext(CGSizeMake(k_SCREEN_WIDTH/2, k_SCREEN_HEIGHT/2));
                         [landscapeImage drawInRect:CGRectMake(0, 0, k_SCREEN_WIDTH/2, k_SCREEN_HEIGHT/2)];
    ...
    [tw_waccountStore requestAccessToAccountsWithType:accountType options:nil completion:^(BOOL granted, NSError *error)
     {
         ...
         if (granted == YES)
         {
             ...
             if (count > 0)
             {
                 ...
                 NSURL *url = [NSURL URLWithString:@"https://api.twitter.com"
                               @"/1.1/statuses/update_with_media.json"];
                 NSDictionary *params = @{@"status" : [NSString stringWithFormat:@"%@ %s https://itunes.apple.com/us/app/id%d %s",feed,TAG_LINE,APPLE_ID]};
                 SLRequest *request = [SLRequest requestForServiceType:SLServiceTypeTwitter
                                                         requestMethod:SLRequestMethodPOST
                                                                   URL:url
                                                            parameters:params];
...
{
    [[PineGameDelegate sharedInstance] PostImageToTwitter:[NSString stringWithFormat:@"%s",feed] withRealUrl:[NSString stringWithFormat:@"%s",realUrlImage]];
...
{
#if USE_PINESOCIAL_AT_ID_STRING
    NSString* url = [[PineGameDelegate sharedInstance] GetURLTwitterAvartar:[NSString stringWithFormat:@"%s",id]];
#else
