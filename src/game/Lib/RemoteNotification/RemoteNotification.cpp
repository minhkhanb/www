...
int send_remote_notification(const char *deviceTokenHex, Payload *payload)
{
        ...
        int ret = send_payload(deviceTokenHex, messageBuff, strlen(messageBuff));
        printf("\nret send payload =%d\n",ret);
}
...
int send_payload(const char *deviceTokenHex, const char *payloadBuff, size_t payloadLength)
{
    ...
    if (sslcon && deviceTokenHex && payloadBuff && payloadLength)
    {
        ...
        *binaryMessagePt++ = command;
        ...
        int ssl_write = SSL_write(sslcon->ssl, binaryMessageBuff, (binaryMessagePt - binaryMessageBuff));
        printf("\nssl_write = %d",ssl_write);
        if (ssl_write > 0)
            rtn = 1;
    }
