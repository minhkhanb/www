...
#ifndef CAPTURE_VIDEO_W
#define CAPTURE_VIDEO_W/2
#endif
#ifndef CAPTURE_VIDEO_H
#define CAPTURE_VIDEO_H/2
#endif
CGSize const DefaultFrameSize                             = (CGSize){CAPTURE_VIDEO_W, CAPTURE_VIDEO_H};
    ...
    while (1)
    {
		if(writerInput.readyForMoreMediaData){
			...
			} else
            {
				...
				buffer = [HJImagesToVideo pixelBufferFromCGImage:[array[i] CGImage] size:CGSizeMake(size.width, size.height)];
			}
                    ...
                    for (double j = 1; j < framesToFadeCount; j++) {
                        ...
                        buffer = [HJImagesToVideo crossFadeImage:[array[i] CGImage]
                                                         toImage:[array[i + 1] CGImage]
                                                          atSize:CGSizeMake(size.width, size.height)
