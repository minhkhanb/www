...
DirectXPanelBase::DirectXPanelBase() :
    ...
    m_height(1.0f),
    m_width(1.0f),
	m_create_device_success(false)
{
	CurrentError = S_OK;
	USE_TEST_SERVER("192.168.1.34", "AvengerWp8");
	...
	//TEST_LOG_RAW("test log");
}

void DirectXPanelBase::ServiceRequest(int request_id){
	OnRequest(request_id);
}
...
void DirectXPanelBase::CreateDeviceResources()
{
	...
	//TEST_LOG_RAW(((rs == S_OK) ? "Create dx device ok" : " Create device fail"));
	if (rs != S_OK)
	{
		//show dialog here
...
void DirectXPanelBase::CreateSizeDependentResources()
{
	if (m_create_device_success){
		...
		else // Otherwise, create a new one.
		{
			...
			swapChainDesc.BufferDesc.Width = static_cast<UINT>(m_renderTargetWidth);
